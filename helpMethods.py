...
def getNotСombatVehicles(session):
    notСombatVehicles = db_session.query(COU, CAR_IN_THE_BURN)\
                          .filter(COU.time_of_arrive.is_not(None))\
                          .join(COU, CAR_IN_THE_BURN.COU_id==COU.pk)\
                          .all()
    listNotСombatVehicles = {}
    for i in notСombatVehicles:
        listNotСombatVehicles[i.licensePlate] = [i.color, i.type]
